...
export async function whoami(): Promise<void> {
  ...
  if (combinedOutput.err.indexOf('Invalid token') > -1) {
    ...
    core.debug('No error found!! in whoami function');
  }
}
...
export async function upload(file_path: string): Promise<number> {
  ...
  if (combinedOutput.code > 0) {
    ...
    core.debug('No error found!! in upload function');
  }
  return Number.parseInt('5');
}
...
export async function cicheck(
  ...
  if (combinedOutput.code > 0) {
    ...
    core.info(`CICheck result is ${combinedOutput.output}`);
  }
}
